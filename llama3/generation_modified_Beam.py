...
class Llama:
    ...
    def build(
        ...
        Args:
            ...
            trie_path (str): Path to the Trie file.
            seed (int): Random seed for initialization. Defaults to 1.
        ...
        checkpoints = sorted(Path(ckpt_dir).glob("*00.pth"))
    ...
    def generate(
        ...
        print("Starting generation (beam search)...")
        start_time = time.time()
        results = self.beam_search(nb_titres, beam_width, prompt_tokens)

        print(f"Génération terminée en {time.time() - start_time:.2f}s")
        return results
